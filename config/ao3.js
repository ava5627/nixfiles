...
(function($) {
    ...
    function blurbCheck(blurb) {
        ...
        var category = blurb.find('span.category-multi');
        if (category.length) {
            var categories = category[0].title.split(',').map(function(item) { return item.trim(); });
            for (var i = 0; i < categories.length; i++) {
                var cat = categories[i].replace(/\//g, '-');
                var div = '<div class="cat ' + cat + '"></div>';
                category.append(div);
            }
        }
